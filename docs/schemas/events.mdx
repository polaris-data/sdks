...
`exchange_timestamp`, and nullable `exchange_sequence` instead. SDK filters and
derived calculations use legacy `timestamp` or v2 `collector_timestamp`.

Exchange time is venue provenance and may be null or regress. Preserve the
order returned by the SDK. See [Event schema v2 migration](/guides/event-schema-v2-migration)
for version-matching examples in every language.

...
Exact batches preserve source-file order and use `timestamp[us, tz=UTC]`.
...
`receive_timestamp`, `sequence`, and `sequence_scope` are nullable because not
every venue provides them. Typed columns cover common trade, order-book, and
...
it contains the resulting complete-book event.
Sequence values are only comparable within their declared scope and may reset;
neither timestamps nor sequences are used to reorder stored events.
...
- `orderbook`: a complete order book snapshot
- `orderbook_delta`: changed price levels; zero quantity removes a level

The SDKs materialize orderbooks by default: deltas become complete `orderbook`

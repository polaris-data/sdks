...
match &event {
    StandardEvent::Legacy(row) => println!("{}", row.timestamp),
    StandardEvent::V2(row) => println!("{:?}", row.exchange_timestamp),
}
println!("SDK time: {}", event.timestamp());
```

See [Event schema v2 migration](/guides/event-schema-v2-migration) for trade,
order-book, and ordering changes.

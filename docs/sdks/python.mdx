...
gaps and reconnects; deltas are skipped until another snapshot arrives. Pass
`materialize_orderbooks=False` for raw deltas, or use the exported
`OrderbookBuilder` directly for application-managed flows.

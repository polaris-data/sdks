...
| `replay(source, market, from_=None, to=None, standard=True, materialize_orderbooks=True)` | Iterator of rows | Stream historical data for replay or backfills |
| `events(source, market, from_=None, to=None, materialize_orderbooks=True)` | Iterator of normalized events | Process mixed event types with bounded memory |
...
| `bbo(source, market, from_=None, to=None, interval=None, output="iterator", batch_size=65536)` | Iterator, Arrow batches, or DataFrame | Spread tracking, quote analytics, and top-of-book monitoring |
